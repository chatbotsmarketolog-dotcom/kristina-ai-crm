...
def hash_pwd(p): 
    return hashlib.sha256((p + "crm_salt_2026").encode()).hexdigest()
...
class ConnManager:
    def __init__(self): 
        self.visitors, self.operators = {}, {}
    
    async def add_vis(self, ws, cid): 
        await ws.accept()
        self.visitors[cid] = ws
    
    async def add_op(self, ws, oid): 
        await ws.accept()
        self.operators[oid] = ws
    
    def rem_vis(self, cid): 
        self.visitors.pop(cid, None)
    
    def rem_op(self, oid): 
        self.operators.pop(oid, None)
        ...
        for o, w in self.operators.items():
            if o == owner_id:
                try: 
                    ...
                    dead.append(o)
        for o in dead: 
            self.operators.pop(o)
    
    async def to_vis(self, cid, data):
        if cid in self.visitors: 
            await self.visitors[cid].send_json(data)
...
@app.get("/auto.js")
async def get_autojs():
    return FileResponse("auto.js", media_type="application/javascript")

# === HEALTH CHECK ===
@app.get("/health")
async def health():
    return {"status": "ok", "message": "CRM is alive"}
...
async def settings_page():
    return open("settings.html", "r", encoding="utf-8").read()

@app.get("/widget/{site_key}", response_class=HTMLResponse)
async def widget_page(site_key: str):
    return open("widget.html", "r", encoding="utf-8").read()
...
async def add_site(req: Request, owner: str = Depends(get_owner_id)):
    ...
    url = d.get("url","").strip()
    if not url: 
        return {"error":"Укажите URL"}, 400
    key = f"site_{uuid.uuid4().hex[:8]}"
    conn = db()
    conn.execute("INSERT INTO websites (url, api_key, owner_id) VALUES (?,?,?)", (url, key, owner))
    conn.commit()
    conn.close()
...
async def get_sites(req: Request, owner: str = Depends(get_owner_id)):
    conn = db()
    rows = conn.execute("SELECT * FROM websites WHERE owner_id=? ORDER BY created_at DESC", (owner,)).fetchall()
    conn.close()
...
async def approve_site(req: Request):
    ...
    conn = db()
    conn.execute("UPDATE websites SET status='active', owner_id='approved' WHERE api_key=?", (api_key,))
    conn.commit()
    conn.close()
    return {"ok": True}
...
async def new_chat(site_key: str):
    ...
    if not site: 
        conn.close()
        return {"error":"Сайт не найден"}, 404
...
async def accept(chat_id: int, req: Request, owner: str = Depends(get_owner_id)):
    conn = db()
    conn.execute("UPDATE chats SET status='active', operator_id=? WHERE id=?", (owner, chat_id))
    conn.commit()
    conn.close()
...
async def ws_vis(ws: WebSocket, chat_id: int):
    ...
    try:
        while True:
            ...
            conn.execute("INSERT INTO messages (chat_id, sender, text) VALUES (?,?,?)", (chat_id, "visitor", d["text"]))
            conn.commit()
            conn.close()
            if owner_row: 
                await mgr.notify_ops(owner_row["owner_id"], {"type":"msg", "chat_id":chat_id, "sender":"visitor", "text":d["text"]})
        ...
        conn.close()
        if owner_row: 
            await mgr.notify_ops(owner_row["owner_id"], {"type":"system", "chat_id":chat_id, "text":"Посетитель покинул чат"})
...
async def ws_op(ws: WebSocket, token: str = None):
    ...
    try:
        while True:
            raw = await ws.receive_text()
            d_msg = json.loads(raw)
            conn = db()
            conn.execute("INSERT INTO messages (chat_id, sender, text) VALUES (?,?,?)", (d_msg["chat_id"], "operator", d_msg["text"]))
            conn.commit()
            conn.close()
